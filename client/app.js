...
obj.setup=function(){	/*Setup runs once when the game starts*/
	...
	jte.draw(this);
};obj.JTEcode=["/*You can put functions here*/",""];obj.JTEsetup=["\t/*Setup runs once when the game starts*/","\t"];obj.JTEupdate=["\t/*Update runs at the fps specified*/","\t","\tjte.draw(this);"];jte.objects.push(obj);var obj=new JTEObject(10,480,160,2,[0,0,0],0,1,'undefined',true,'Fuelgame','[""]',false,-1,'Obj162');/*You can put functions here*/
...
obj.setup=function(){	/*Setup runs once when the game starts*/
	...
	jte.draw(this);
};obj.JTEcode=["/*You can put functions here*/",""];obj.JTEsetup=["\t/*Setup runs once when the game starts*/","\t"];obj.JTEupdate=["\t/*Update runs at the fps specified*/","\t","\tjte.draw(this);"];jte.objects.push(obj);var obj=new JTEObject(-90,500,40,40,[127,127,127],0,1,'{"img":"bumper"}',true,'Fuelgame','["Bumper"]',false,-1,'Obj491');/*Attributes and methods go here*/
...
obj.setup=function(){	/*Setup runs once when the game starts*/
  ...
  for(var i=0;i<this.fuels.length;i++){
	...
	jte.draw(this);
};obj.JTEcode=["/*You can put functions here*/","","obj.fuels=[];","obj.timerMax=120;","obj.rotationSpeed=3;","obj.fuelRatio=0.25;",""];obj.JTEsetup=["\t/*Setup runs once when the game starts*/","\tthis.alpha=0;","  this.fuels=jte.getObjects([\"Fuel\"])","  for(var i=0;i<this.fuels.length;i++){","   \t\tthis.fuels[i].timer=0;     ","  }","  "];obj.JTEupdate=["\t/*Update runs at the fps specified*/","\tfor(var i=0;i<this.fuels.length;i++){","    if(this.fuels[i].timer>0){","      this.fuels[i].timer--;","      if(this.fuels[i].timer<=0){","       \tthis.fuels[i].tags=[\"Fuel\"]; ","       \tthis.fuels[i].alpha=1;         ","      }","    }else{","       this.fuels[i].r+=this.rotationSpeed;","      \tvar p=jte.getObject(\"Player2\");","      \tif(jt.cRect(p,this.fuels[i])){","          jt.stopPlay(\"refill\")","         \tthis.fuels[i].timer=this.timerMax;","          this.fuels[i].alpha=0;","          p.fuel+=p.fuelMax*this.fuelRatio;","          if(p.fuel>p.fuelMax){p.fuel=p.fuelMax}","        }","    }","  }","  ","\tjte.draw(this);"];jte.objects.push(obj);var obj=new JTEObject(60,10,110,20,[0,0,0],0,1,'undefined',false,'Fuelgame','[""]',false,-1,'Obj16');/*You can put functions here*/
...
obj.setup=function(){	/*Setup runs once when the game starts*/
  ...
  var w=30;
  var h=10;  
  jt.rect(this.x+this.w/2-w/2,this.y-this.h,w,h,"black")
  jt.rect(this.x+border+this.w/2-w/2,this.y-this.h+border,(w-border*2)*fuelPercent,h-border*2,"red")  
  ...
  client.clientObj.time=this.time;      
  ...
  for (var i = 0; i < len; i++) {
    ...
    if(inCam){
      ...
      jt.text(other.fuel+"%",other.x+this.w/2,other.y-jt.fontSize(),"black","center");   
    }else{
      var dist=jt.distP(this.x,this.y,other.x,other.y);
      ...
      jt.text(other.fuel+"%",posX,posY,"black","center"); 
...
obj.clientObj={name:"",sprite:"",x:0,y:0,dir:1,fuel:0,time:0,state:"",playing:false};
obj.serverObjs={};
...
obj.setup=function(){	/*Setup runs once when the game starts*/
  ...
  if(this.updateCooldown<=0){
	...
	//jt.drawObject(this);
};obj.JTEcode=["/*Attributes and methods go here*/","obj.socketId=undefined;","","obj.fps=1000/30;","","obj.clientObj={name:\"\",sprite:\"\",x:0,y:0,dir:1,fuel:0,time:0,state:\"\",playing:false};","obj.serverObjs={};","","obj.lobbies=[];","","obj.updated=false;","","obj.sent=false;","obj.sent2=false;","obj.inserted=false;","","obj.highscores=[];","","obj.updateCooldown=0;","obj.updateCooldownMax=2;","","obj.started=undefined;","obj.playing=undefined;","obj.inviteSent=undefined;","obj.inviteReceived=undefined;","","obj.playings=[];","obj.withs=[];","obj.received=[];","obj.receivedMax=[];","","obj.isHost=false;","obj.host=undefined;","","obj.index=0;","","obj.socket = {"," \ton:function(){","    ","  },","  emit:function(){","    ","  }","}","","obj.connected=false;",""];obj.JTEsetup=["\t/*Setup runs once when the game starts*/","\tif(window[\"io\"]!==undefined){","    console.log(\"io exists\");","    this.socket=io();","  }","  ","  this.socket.on(\"connected\",function(id,num){","    jt.getObject(\"Client\").connected=true;","    jt.getObject(\"Client\").socketId=id;    ","    jt.getObject(\"Client\").clientObj.name=\"Guest \"+num;    ","  });","  ","  jt.debug(true);","  jt.mute(true);  ","  ","  this.pause=false;","  ","  this.socket.on(\"getData\",function(senderId,serverObj){","    jt.getObject(\"Client\").serverObjs[senderId]=serverObj;","    jt.getObject(\"Client\").updated=true;","    jt.getObject(\"Client\").gotAData=true;","  })","  ","  this.socket.on(\"chat message\",function(msg,color){","    jt.getObject(\"Chat\").messages.push(msg);","    jt.getObject(\"Chat\").messagesC.push(color);    ","  })"];obj.JTEupdate=["\t/*Update runs at the fps specified*/","  this.socket.on(\"disconnected\",function(senderId){","    ","    delete jt.getObject(\"Client\").serverObjs[senderId];","  })","\t","  if(this.updateCooldown<=0){","    ","    if(this.connected){","      this.updateCooldown=this.updateCooldownMax;","      this.clientObj.playing=true;","      this.socket.emit(\"update\",this.clientObj);","    }","  }else{","   \tthis.updateCooldown--; ","  }","  ","\t//jt.drawObject(this);"];jte.objects.push(obj);var obj=new JTEObject(20,180,390,40,[0,0,0],0,1,'{"text":"Current username:","size":24,"align":"center","font":"Consolas"}',true,'Start','[""]',false,-1,'Name');/*Attributes and methods go here*/
...
obj.setup=function(){	/*Setup runs once when the game starts*/
	...
	jte.draw(this);
};obj.JTEcode=["/*Attributes and methods go here*/",""];obj.JTEsetup=["\t/*Setup runs once when the game starts*/","\t"];obj.JTEupdate=["\t/*Update runs at the fps specified*/","\t","\tjte.draw(this);"];jte.objects.push(obj);var obj=new JTEObject(720,0,80,30,[0,0,0],0,1,'{"text":"v0.7","size":23,"font":"Consolas","align":"right"}',true,'Start','[""]',false,-1,'Obj16');/*Attributes and methods go here*/
...
obj.setup=function(){	/*Setup runs once when the game starts*/
	...
	jt.drawObject(this);
};obj.JTEcode=["/*Attributes and methods go here*/",""];obj.JTEsetup=["\t/*Setup runs once when the game starts*/","\t"];obj.JTEupdate=["\t/*Update runs at the fps specified*/","\t","\tjt.drawObject(this);"];jte.objects.push(obj);var obj=new JTEObject(960,-100,310,90,[255,0,0],0,1,'{"text":"Keyboard","size":64,"font":"Consolas","align":"left"}',true,'Start','[""]',false,145,'keyboard');/*Attributes and methods go here*/
/* HOW TO USE
